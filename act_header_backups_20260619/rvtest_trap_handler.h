...
common_\__MODE__\()excpt_handler:
        ...
        la      T2, saved_mepc
        SREG    T3, 0(T2)
        mv      T3, T5                               // T5 already holds xCAUSE from trap entry
        la      T2, saved_mcause
        SREG    T3, 0(T2)
        csrr    T3, CSR_XTVAL
        la      T2, saved_mtval
...
.ifc \__MODE__ , M
        ...
        csrr    T3, CSR_PMPADDR0
        la      T2, saved_pmpaddr0
        SREG    T3, 0(T2)
#endif
#if UDB_NUM_PMP_ENTRIES > 1
        csrr    T3, CSR_PMPADDR1
        la      T2, saved_pmpaddr1
        SREG    T3, 0(T2)
#endif
#if UDB_NUM_PMP_ENTRIES > 2
        csrr    T3, CSR_PMPADDR2
        la      T2, saved_pmpaddr2
        SREG    T3, 0(T2)
#endif
#if UDB_NUM_PMP_ENTRIES > 3
        csrr    T3, CSR_PMPADDR3
        la      T2, saved_pmpaddr3
        SREG    T3, 0(T2)
#endif
#if UDB_NUM_PMP_ENTRIES > 4
        csrr    T3, CSR_PMPADDR4
        la      T2, saved_pmpaddr4
        SREG    T3, 0(T2)
#endif
#if UDB_NUM_PMP_ENTRIES > 5
        csrr    T3, CSR_PMPADDR5
        la      T2, saved_pmpaddr5
        SREG    T3, 0(T2)
#endif
#if UDB_NUM_PMP_ENTRIES > 6
        csrr    T3, CSR_PMPADDR6
        la      T2, saved_pmpaddr6
        SREG    T3, 0(T2)
#endif
#if UDB_NUM_PMP_ENTRIES > 7
        csrr    T3, CSR_PMPADDR7
        la      T2, saved_pmpaddr7
        SREG    T3, 0(T2)
#endif
.endif
        // Restore xEPC into T3 after the debug CSR snapshot above. The relocation
        // checks below consume T3 as the original faulting PC.
        la      T2, saved_mepc
        LREG    T3, 0(T2)
